...
public class Main extends javax.swing.JFrame {
    ...
    private void loadSVGElements () {
        ...
        double b = pA.distance(pC);//Util.distance(pA,pC);
        double c = pA.distance(pB);//Util.distance(pA,pB);
        ...
        for (short tt = 0; tt < 2; tt++) {
            if (pressed[tt]) {
                ...
                if (state[tt] == State.INITIAL || state[tt] == State.TRANSLATED) {
                    if (baricenter[tt].distance(originalBaricenter[tt]) < snapDistance) {
                ...
                double opacity = Math.min(1, baricenter[tt].distance(originalBaricenter[tt])/fullOpacityDistance);
